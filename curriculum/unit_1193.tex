...
When considering the expression, $\int\limits_a^b f(x) dx$ (read "the integral from $a$ to $b$ of the $f$ of $x$ $dx$"), the function $f$ is called the \textit{integrand} and the interval $[a,b]$ is the interval of integration. Also, $a$ is called the \textit{lower limit} and $b$ the \textit{upper limit} of integration.
...
One important feature of this definition is that we also allow functions that take negative values. If $f(x) < 0$ for all $x$ then $f(x_i^*) < 0$ so $f(x_{i}^\ast) \Delta x < 0$. So the definite integral of $f$ will be strictly negative. More generally, if $f$ takes on both positive and negative values then $\int\limits_a^b f(x) dx$ will be the area under the positive part of the graph of $f$ \textbf{minus} the area above the graph of the negative part of the graph (see \href{https://upload.wikimedia.org/wikipedia/commons/thumb/f/f8/Signed-area.png/300px-Signed-area.png}{Figure 4}). For this reason, we say that $\int\limits_a^b f(x) dx$ is the \textbf{signed area} under the graph.
...
\begin{align}
\int\limits_a^b x^2 dx &= (b-a)\left(a^2 + a(b-a) + \frac{1}{3}(b-a)^2\right) \\\&= (b-a)\left(a^2 + ab - a^2 + \frac{1}{3}(b^2 - 2ab + a^2)\right) \\\&= \frac{1}{3}(b-a)(b^2 + ab + a^2) \\\&= \frac{1}{3}(b^3 - a^3)
\end{align}
...
\textbf{Constant Rule:} $\int\limits_a^b c \cdot f(x) dx = c \int\limits_a^b f(x) dx$
...
If $c$ is constant then $\int\limits_a^b c \, dx = c(b-a)$
...
From above $\int\limits_1^3 9 dx = 18$ and $\int\limits_1^3 2x^2 dx = \frac{52}{3}$ so:
$$
...
Suppose $f(x)$ is continuous on $[a,b]$. Then $\frac{1}{b-a}\int\limits_a^b f(x)dx = f(c)$ for some $c \in [a,b]$.
...
&= \int\limits_0^h \pi r^2 dx\\\\
&= \pi r^2 \int\limits_0^h dx\\\\
...
&= \int\limits_0^h \pi \frac{R^2}{h^2}(h-x)^2 dx\\\\
&= \pi \frac{R^2}{h^2} \int\limits_0^h (h-x)^2 dx
\end{align}

By u-substitution, we may let $u = h-x$, then $du = -dx$ and our integral becomes

\begin{align}
&= \pi \frac{R^2}{h^2} \left(-\int\limits_h^0 u^2 du\right)\\\\
...
&= \pi \int\limits_{-R}^R R^2 dx - \pi \int\limits_{-R}^R x^2 dx\\\\
...
\paragraph{Steps}
\begin{align}
\int\limits_{x=a}^{x=b}f(x)dx &= \int\limits_{x=a}^{x=b} f(x)\ \frac{du}{du}\ dx & (1) \text{i.e.} \quad \frac{du}{du}=1 \\\\
&= \int\limits_{x=a}^{x=b}{\left(f(x)\ \frac{dx}{du}\right)\left(\frac{du}{dx}\right)}\ dx & (2) \text{i.e.} \quad \frac{dx}{du}\cdot\frac{du}{dx}=1 \\\\
&= \int\limits_{x=a}^{x=b}\left(f(x)\ \frac{dx}{du}\right)g''(x)\ dx & (3) \text{i.e.} \quad \frac{du}{dx}=g''(x) \\\\
&= \int\limits_{x=a}^{x=b}h(g(x))g''(x)dx & (4) \text{i.e.} \quad \text{Now equate } \left(f(x)\ \frac{dx}{du}\right) \text{ with } h(g(x)) \\\\
&= \int\limits_{x=a}^{x=b}h(u)g''(x)dx & (5) \text{i.e.} \quad g(x)=u \\\\
&= \int\limits_{u=g(a)}^{u=g(b)}h(u)du & (6) \text{i.e.} \quad du=\frac{du}{dx}dx=g''(x)dx \\\\
&= \int\limits_{u=c}^{u=d}h(u)du & (7) \text{i.e.} \quad \text{We have achieved our desired result} \\\\
\end{align}
